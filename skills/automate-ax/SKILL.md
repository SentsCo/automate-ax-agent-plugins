...
- Use `waitForResponse: true` with `respondToHttpRequest` only when the caller needs the result and the work can finish within the HTTP response window. Otherwise, accept the request asynchronously. Inspect the completed run afterward.
